...
from .config import METADATA_JSON, PAPERS_JSONL, SEARCH_QUERIES
...
def _load_metadata(path: Path = METADATA_JSON) -> List[Dict]:
    """Load the single metadata.json array (empty if missing/invalid)."""
    if not path.exists():
        return []
    try:
        with path.open("r", encoding="utf-8") as f:
    ...
    if isinstance(data, dict) and "papers" in data and isinstance(data["papers"], list):
        return data["papers"]
    logger.warning("Unexpected metadata format in %s — starting fresh", path)
    return []


def _save_metadata(record: Dict, path: Path = METADATA_JSON) -> None:
    """Upsert one record into the single metadata.json file."""
...
def rebuild_metadata_from_jsonl(
    jsonl_path: Path = PAPERS_JSONL,
    metadata_path: Path = METADATA_JSON,
) -> int:
    """Rebuild metadata.json as one array from papers.jsonl. Returns count."""
    records: List[Dict] = []
    ...
    with metadata_path.open("w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=2)
    logger.info("Rebuilt %s with %d records", metadata_path, len(records))
    return len(records)


def append_record(record: Dict, path: Path = PAPERS_JSONL) -> None:
    """Append one paper record to papers.jsonl and upsert into metadata.json."""
    path.parent.mkdir(parents=True, exist_ok=True)
